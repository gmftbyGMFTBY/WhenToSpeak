...
class Utterance_encoder_gn(nn.Module):
    ...
    def __init__(self, input_size, embedding_size, 
        ...
        super(Utterance_encoder_gn, self).__init__()
...
class GCNRNNContext(nn.Module):
    ...
    def forward(self, gbatch, utter_hidden):
        # befor generating the batch, change the utter_hidden by RNN
        rnn_x, _ = self.rnn(utter_hidden)    # [turn, batch, hidden]
...
class W2T_GCNRNN(nn.Module):
    ...
    def __init__(self, input_size, output_size, embed_size, utter_hidden_size, 
        ...
        super(W2T_GCNRNN, self).__init__()
        self.teach_force = teach_force
        self.output_size = output_size
        self.pad, self.sos = pad, sos
        self.utter_encoder = Utterance_encoder_gn(input_size, embed_size,
                                                  utter_hidden_size,
                                                  dropout=dropout,
                                                  n_layer=utter_n_layer) 
        self.gcncontext = GCNRNNContext(utter_hidden_size+user_embed_size, 
                                        context_hidden_size,
                                        context_hidden_size, 
                                        position_embed_size, dropout=dropout)
        self.decoder = Decoder_gn(output_size, embed_size, 
                                  decoder_hidden_size, 
